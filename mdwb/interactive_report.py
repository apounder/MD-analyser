    ...
    for key, group in groups.items():
        ...
        if categorical:
            ...
            if circular:
                ...
                nonempty = [a for a in finite_arrays if len(a)]
                low = min((float(a.min()) for a in nonempty), default=0)
                high = max((float(a.max()) for a in nonempty), default=1)
                if low == high:
        ...
        for series, finite in zip(group, finite_arrays):
            ...
            if categorical:
                ...
                data = (finite + 180) % 360 - 180 if circular else finite
...
def write_interactive_report(config, manifest, summary, output):
    ...
    for filename in summary.get("figures", []):
        ...
        if used + size <= 20_000_000:
            ...
            used += size
        figures.append({"path": relative.as_posix(), "analysis": summary.get("figure_analyses", {}).get(filename, ""), "image": embedded})
